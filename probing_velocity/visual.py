...
class Environment(object):
    def __init__(self, lane_list, agent_list, state):
    ...
    def step(self, new_state):
        ...
        pos = utils.state_to_plot(new_state)
        
        # next_speed, next_heading, lane_angle, lane_number, all_lane distance from center, off_road, opp_laneNum, opp_dist
        for l in self.lane_list:
            l.relative_move([0, (new_state[1] - self.state[1])* utils.real_to_plot_ratio])
        
        # move autonomous agent
        self.agent_list[0].move([0, pos[0] - self.agent_list[0].mutable_bb.center[1]])
        self._renderObj()
