    ...
    def generate_control(self, state):
        match self.phase:
            case 0:
                ...
                if (state.x[1] <= state.x[self.target - 1] and state.lane[1]==0):
                    self.phase = 1
                reward = []
                for i in config.ego_action_space:
                    ...
                    reward.append(-(new_state.v[self.id] - self.cutoff_speed)**2 - (i - 0)**2 - 2 * (state.u[0]-i)**2 )
            ...
            case 1:
                ...
                for i in config.ego_action_space:
                    ...
                    controls[self.id] = i
                    new_state = state.update(controls, config.d_t)
                    reward.append((i/1)*(new_state.v[self.id] - 20)**2 - abs(i) - (i - 0)**2 - 2 * (state.u[0]-i)**2 )
                pos = np.argmax(reward)
                return config.ego_action_space[pos]
