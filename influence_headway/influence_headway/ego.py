    ...
    def generate_control(self, state):
        match self.phase:
            case 0:
                ...
                if ((state.x[self.head_id] - state.x[self.id]) >= 5):
                    state.lane[0] = 0
                    self.phase = 1
                ...
                for i in config.ego_action_space:
                    ...
                    reward.append(-(new_state.x[self.head_id] - new_state.x[self.id] - 10)**2)
            ...
            case 1:
                ...
                for i in config.ego_action_space:
                    ...
                    controls[self.id] = i
                    new_state = state.update(controls, config.d_t)
                    reward.append(-(new_state.x[self.head_id] - new_state.x[self.id] - self.phi)**2 - (new_state.v[self.head_id] - new_state.v[self.id])**2)
                pos = np.argmax(reward)
                return config.ego_action_space[pos]
            case 2:
                reward = []
