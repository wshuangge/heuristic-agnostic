...
def state_to_plot(state):
    ...
    relative = [each + 400 for each in relative]
    return relative
...
def images_to_video():
    ...
    pathIn= './frames/'
    pathOut = './videos/video1_baseline.avi'
    fps = 40
    frame_array = []
...
def plot_deviation(x, y, state, timer):
    ...
    labels = ['Autonomous Vehicle (Yellow)', 'Human Vehicle (Orange)', 'Background Vehicle 1 (White)', 'Background Vehicle 2 (White)']
...
def plot_velocity(x, y, state, timer):
    ...
    if (timer >= 260):
        figure.savefig("./figures/velocity_baseline_2.jpg", dpi=300)
...
def plot_deviation(x, y, state, timer):
    ...
    if (timer >= 260):
        fig.savefig("./figures/deviation_baseline_2.jpg", dpi=300)
